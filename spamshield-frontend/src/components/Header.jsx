...
const Header = ({ onStatusClick, showBackButton, onBackClick }) => {
  return (
        ...
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            {showBackButton ? (
              <button
                onClick={onBackClick}
                className="mr-4 px-3 py-2 bg-white bg-opacity-20 hover:bg-opacity-30 rounded-md text-sm font-medium transition-colors"
              >
                ← Back to Main
              </button>
            ) : null}
